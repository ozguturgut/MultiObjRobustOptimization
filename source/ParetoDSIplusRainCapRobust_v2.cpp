    ...
    try {
        ...
        for (int wcount = 0; wcount < (nPareto + 1); ++wcount) {
            ...
            for (int s = 0; s < nScenarios; s++)
            {
                ...
                model.addConstr(ptot - KizilirmaktCap <= maxExtraNeed, cname.str());
            }
            ...
            model.write("ParetoDSIplusRainCapRobust.lp");
            //model.write("watermodel.mps");
    //**************************************************************************OUTPUTS**************************************************************************
            std::ofstream myfile;
            ostringstream paretofilename;
            paretofilename << "ParetoDSIplusRainCapRobust_P" << wcount << ".csv";
