    ...
    char* argv[])
{
    try {
        ...
        double KizilirmaktCap = 7980000000;   //Total for region per month in kg
        ...
        for (int wcount = 0; wcount < (nPareto + 1); ++wcount) {
            ...
            for (int p = 0; p < nPeriods; ++p)
            {
                ...
                for (int s = 0; s < nScenarios; ++s)
                {
                    ...
                    //extraneed[p][s].set(GRB_DoubleAttr_UB, (10000 * TargetWatCap[p]));
            ...
            for (int p = 0; p < nPeriods; ++p) {
                ...
                for (int c = 0; c < nCrops; c++) {
                    ...
                    for (int s = 0; s < nScenarios; s++) {
                        ...
                        //totwatercons[p][c][s].set(GRB_DoubleAttr_Obj, (weightwater) / (TargetWatCap));
                    }
                }
            }
            ...
            for (int s = 0; s < nScenarios; s++) {
                ...
                grndwatercons[s].set(GRB_DoubleAttr_Obj, (weightwater) / (12*KizilirmaktCap));

            }
            GRBVar maxBudgetDeficit = model.addVar(0.0, budgetdeficitUB, (weightrevenue / budgetdeficitUB), GRB_CONTINUOUS, "maxBudgetDeficit");;


            GRBVar maxExtraNeed = model.addVar(0.0, KizilirmaktCap, (weightwater / KizilirmaktCap), GRB_CONTINUOUS, "maxExtraNeed_P");
            ...
            for (int p = 0; p < nPeriods; ++p)
            {
                for (int s = 0; s < nScenarios; s++)
                {
                    ...
                    ostringstream cname;
                    cname << "DeviateWatTar.P" << p << ".S" << s;
                    //cout << "p:" << p << " s:" << s << endl;
                    //if(p==26 && s==26){
                   //     cout << "param:" << TargetWatCap[p] << " multip:" << 1000 * TargetWatCap[p] << endl;
                   // }
                    model.addConstr(ptot == 1000 * TargetWatCap[p], cname.str()); //TargetWatCap is read from file in tons
                }
            ...
            for (int s = 0; s < nScenarios; s++)
            {
                ...
                model.addConstr(ptot   <= KizilirmaktCap, cname.str());
            }
            ...
            model.write("ParetoDSIplusRainCapReg.lp");
            //model.write("watermodel.mps");
    //**************************************************************************OUTPUTS**************************************************************************
            std::ofstream myfile;
            ostringstream paretofilename;
            paretofilename << "ParetoDSIplusRainCapReg_P" << wcount << ".csv";
